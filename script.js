...
updateThemeIcon();
updateProfileBg(shouldUseDarkTheme);

function updateProfileBg(isDark) {
  const profileSection = document.getElementById("profile");
  if (!profileSection) return;
  profileSection.classList.remove("profile-dark-bg", "profile-light-bg");
  profileSection.classList.add(isDark ? "profile-dark-bg" : "profile-light-bg");
}
...
menuLinks.forEach(link => {
  link.addEventListener("click", e => {
    ...
    if (!targetId) return; // Only handle links with data-target
    e.preventDefault();
